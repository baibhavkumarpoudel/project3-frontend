...
class Player extends Component {
    ...
    componentDidMount = () => {
        axios.get(`https://localhost:3001/player/${this.props.match.params.id}`)
        .then(resp => {
            this.setState({  
                player: resp.data
            })
    ...
    handleSubmit = (event) => {
        ...
        axios.put(`https://localhost:3001/player/${this.props.match.params.id}`, this.state.player)
        .then(response => {
    ...
    handleDelete = (event) => {
        event.preventDefault()
        axios.delete(`https://localhost:3001/player/${this.props.match.params.id}`)
        .then((props) => {
